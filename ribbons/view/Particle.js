...
var Particle = function()
{
    ...
    this.maxThickness = Utils.random(.3, .5);
    this.trailPoints  = Utils.random(4, 10) >> 0;
...
{
    this.startY   = window.PARAMS.randomY ? Utils.random(-15, 15) : 0
    // this.startY = -10;
    this.position = new THREE.Vector3(this.startX, this.startY, this.depth );
...
Particle.prototype.update = function(dt) 
{
    ...
    diff.add(new THREE.Vector3( 
        Utils.random(-.5, .5) * dimX,
        Utils.random(-.5, .5)*dimY,
        Utils.random(-.5, .5)*dimZ)
    );
